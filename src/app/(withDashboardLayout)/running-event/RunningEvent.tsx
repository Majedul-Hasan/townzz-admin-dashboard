...
const RunningEvent = () => {
  ...
  const { data: concertTable, isLoading } = useGetAllEventsQuery({
    page,
    limit,
    status: 'UPCOMING',
  });

  const button = concertTable && [
    ...Array(concertTable?.data?.meta?.totalPage).keys(),
  ];
  return (
    <div className='p-10'>
      ...
      <div>
        <ConcertTable
          concertTable={concertTable?.data?.data}
          isLoading={isLoading}
          serial={page * limit - limit}></ConcertTable>
      </div>
